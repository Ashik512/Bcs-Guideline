...
<div class="col-md-6 mt-4">
            ...
            <div class="card card-primary">
              ...
              <!-- /.card-header -->
              <!-- form start -->
              <form role="form" method="POST" action="{{route('update-model-question',$question->id)}}">
                @csrf
                  ...
                  <div class="form-group">
                    <label>Model Test Name</label>
                      <div class="col-sm-9">
                        ...
                        <select class="form-control" name="model_code" required >
                          <option value="" disabled selected>Please select Model Test.</option>
                                  
                          @foreach($models as $model)
                          @if($question->model_code == $model->model_code)
                          <option value="{{ $model->model_code }}" selected> {{ $model->model_name }} </option>
                          @endif
                          @if($question->model_code != $model->model_code)
                          <option value="{{ $model->model_code }}"> {{ $model->model_name }} </option>
                          @endif
                          @endforeach
